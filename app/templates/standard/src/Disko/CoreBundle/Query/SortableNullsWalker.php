...
/**
 * CoreBundle Vendor Specific DQL Query Features
 *
 ...
 *
 * $query = $qb->getQuery();
 * $query->setHint(Doctrine\ORM\Query::HINT_CUSTOM_OUTPUT_WALKER, 'Webges\CoreBundle\Query\SortableNullsWalker');
 * $query->setHint("sortableNulls.fields", array(
 * 				"p.firstname" => Webges\CoreBundle\Query\SortableNullsWalker::NULLS_FIRST,
 * 				"p.lastname"  => Webges\CoreBundle\Query\SortableNullsWalker::NULLS_LAST,
 * 				"p.basedOnPerson.id" => Webges\CoreBundle\Query\SortableNullsWalker::NULLS_LAST
 * 			));
 *
...
namespace Disko\CoreBundle\Query;
